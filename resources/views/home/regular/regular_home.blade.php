...
$btns = [
    ...
    create_button('btn_transfer', __('home_lng.transfer'), "btn btn-success $pull", 'margin-left:2px;margin-right:2px;', 'fa fa-rocket', '', 'onclick="transfer()"')
];
...
begin_modal('modal_transfer');
    echo Form::open(['id'=>'form_transfer', 'action' => ['HomeController@transfer_packet'], 'method'=>'POST','enctype'=>'multipart/form-data']);
        ...
        begin_row();
            create_input_group('operator', __('home_lng.operator'), 'fa fa-building', 'select', $select_operators);
        end_row();
        begin_row();
